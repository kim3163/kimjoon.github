...
class IrisHTML(object):
	...
	def get_iris_html(self):
		...
		for ip in self.ip_list:
			...
			irisOpenLab = self.raw_dict['openlab.bigdata-transportation.kr']['IRIS_OPENLAB']['VALUE']
			selenium    = self.raw_dict['openlab.bigdata-transportation.kr']['WEB-LOGIN']['VALUE']

			if iris_flag:
				iris_flag=False
				mergelist.append("<h1 align='center'>IRIS</h1>")
				mergelist.append("<h2 align='center'>IRIS LISTENER STATUS : " + irisStatus + "</h2>")
				mergelist.append("<h2 align='center'>IRIS OPENLAB STATUS : " + str(irisOpenLab) + "</h2>")
				mergelist.append("<h2 align='center'>WEB-LOGIN STATUS : " + selenium + "</h2>")
				...
				if irisOpenLab != 200  :	
					status_nok_count+=1
					strNoklist.append("Iris OpenLab Error <br>")
				if selenium == 'NOK' :
					status_nok_count+=1
					strNoklist.append("WEB-LOGIN Error <br>")
			try:
				hostname=''
