...
from Noti.ExceptionHTML import *
...
class SendEmail(object):
	...
	def get_emails_config(self):
		try :
			...
			config_smtpport = int(self.config.get('EMAIL','SMTP_PORT'))
		...
		try:
			...
			for ip in sorted(self.raw_dict.keys()):
				...
				if 'FILE' in self.raw_dict[ip].keys():
					file_list.append(ip)
				if 'EF' in self.raw_dict[ip].keys():
					EF_list.append(ip)
				if 'EXCEPTION' in self.raw_dict[ip].keys():
					exception_list.append(ip)
			...
			if len(exception_list) > 0:
				mException = ExceptionHTML(self.raw_dict, self.config, sorted(exception_list, key=natural_keys))
				status_nok_count, nok_list, merge_list = mException.get_exception_html()
				self.status_nok_count += status_nok_count
				self.merge_list+=merge_list

	#		__LOG__.Trace(self.merge_list)
