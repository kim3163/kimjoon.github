	...
	def get_EF_html(self):
		...
		for ip in self.ip_list:
			...
			try:
				...
				for port in self.raw_dict[ip]['EF'] :
					#__LOG__.Trace(port)
					...
					for ps in self.raw_dict[ip]['EF'][port]:
						abn = ps['VALUE']['ABN']
						status = ps['VALUE']["STATUS"]
						ps_name = ps['VALUE']["NODE"]
						queue_count = ps['VALUE']['QUEUE_COUNT']
						...
						if ps['STATUS'] == 'OK' :
							...
							status_nok_count += 1
							strNoklist.append("[EventFlow Status] Port[%s] / Node[%s] / ABN[%s] / Queue[%s] <br>" % (port, ps_name, abn, queue_count))
