...
class DBLoader():
	...
	def getEmsData(self, oneEqpInfoDict) :
		...
		if self.jsonValue(oneEqpInfoDict, 'emsIp') != None and self.jsonValue(oneEqpInfoDict, 'emsIp') != '' :
		...
		if ( vendor != None and vendor != '' ) and  vendor == self.ANIMP_CD   :
			...
			eqpEmsNm	= self.IRISDB.selectIpEqpData( svrIpList, tacsEqpId )
...
#				expEmsNm, expEmsIp, vendor, eqpTyp	= self.IRISDB.selectNmEmsData( emsNm )
	...
	def parseEqpData (self, oneEqpInfoDict) :
		...
		if self.jsonValue( oneEqpInfoDict, 'cmdWorkTypCd' ) == '4' :
			oneEqpInfoDict['eqpId']		= self.jsonValue( oneEqpInfoDict, 'enbId' )

		# Animp 의 cmdWorkTypCd = 7
		if self.jsonValue( oneEqpInfoDict, 'vendor' ) == self.ANIMP_CD or self.jsonValue( oneEqpInfoDict, 'eqpId' ) != '':
			expValidYn = 'Y'
