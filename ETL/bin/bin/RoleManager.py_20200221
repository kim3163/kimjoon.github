...
class RoleManager:
	def __init__(self, cfg, section):
		...
		self.scheduler.add_job(self.checkRoleStatus, 'cron', hour='6', minute='0', second='0', id='RoleCheckManager')
	...
	def getAllUser(self, host, session) :
		...
		result = '[]'
		try :
			code, result = self.restAPI.execute(host, 'GET', uri, None, session)
		except :
			__LOG__.Exception()
