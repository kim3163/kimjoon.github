...
#			__LOG__.Trace(tacsEqpId)
			...
			# --- 2020 05 21 김준우 추가 HOLA 연동 ---
			if ( vendor != None and vendor != '' ) and vendor == self.HOLA_CD :
...
#				expEmsNm, expEmsIp, vendor, eqpTyp	= self.IRISDB.selectNmEmsData( emsNm )
	...
	def parseEqpData (self, oneEqpInfoDict) :
		...
		if self.jsonValue( oneEqpInfoDict, 'vendor' ) == self.ANIMP_CD or self.jsonValue( oneEqpInfoDict, 'vendor' ) == self.HOLA_CD or ( self.jsonValue( oneEqpInfoDict, 'eqpId' ) != '' and self.jsonValue( oneEqpInfoDict, 'eqpId' ) != None ):
			expValidYn = 'Y'
