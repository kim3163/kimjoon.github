...
class WorkInfoCollector :
	...
	def _requestGet(self, url, verify = False) :
		rawDict 	= None
		response	= None
		global REQUEST_FAIL_CNT

		try :
			response 	= requests.get(url = url, headers = self.headers, verify = verify)
			
			if response != None and response.status_code == 200 :
				if REQUEST_FAIL_CNT != 0 :
					endDateObj 	= datetime.now()
					endDate		= endDateObj.strftime('%Y%m%d%H%M')

					startDateObj	= endDateObj - timedelta(minutes = REQUEST_FAIL_CNT)
					startDate		= startDateObj.strftime('%Y%m%d%H%M')
					migrationResult = os.popen('sh %s %s %s' % (self.migrationPath, startDate, endDate))
					REQUEST_FAIL_CNT = 0

					if migrationResult in 'ERROR' :
				...
				REQUEST_FAIL_CNT = REQUEST_FAIL_CNT + 1
				pass
