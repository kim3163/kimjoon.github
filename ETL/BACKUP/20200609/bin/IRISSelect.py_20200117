...
class IRIS_SQL :
	...
	def selectIpEqpData ( self, svrIpList, tacsEqpId ) :
		__LOG__.Trace('svrIp = %s | parentEqpId : %s ' % (svrIpList, tacsEqpId) )
		conn	= None
		result	= ''
		try :
			conn 	= M6.Connection(self.IRIS, self.IRIS_ID, self.IRIS_PASS, Database = 'tacs' )
			...
			sql = """
					...
					WHERE 
						...
						EMS_EQP_ID = '%s'
					LIMIT 1
						""" % (ipList, ipList, ipList, ipList, ipList, ipList, tacsEqpId)
