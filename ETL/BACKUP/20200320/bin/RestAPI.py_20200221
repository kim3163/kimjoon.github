...
class RestAPI:
	...
	def execute(self, host, method, uri, param=None, session=None) :
		if session == None :
		...
		try :
			...
			r = None
			if param is None :
				r = session.request(method, 'https://%s%s' % (host, uri), verify = DOMAIN_PKI_REPO[host]) 
			else :
				r = session.request(method, 'https://%s%s' % (host, uri), verify = DOMAIN_PKI_REPO[host], json = param) 
