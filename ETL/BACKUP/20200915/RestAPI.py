...
ENM_BUSAN1_DOMAIN = '192.168.102.253'
...
class RestAPI:
	...
	def getELGSession(self, host) :
		...
		res = session.post('https://%s:5550/login' % host, data={'IDToken1' : USER_INFO_REPO[host]['PARAM1'], 'IDToken2' : USER_INFO_REPO[host]['PARAM2']}, verify=DOMAIN_PKI_REPO[host], allow_redirects=False)
	...
	def execute(self, host, method, uri, param=None, session=None, timeout=None) :
		...
		try :
			...
			r = None
			if param is None :
				r = session.request(method, 'https://%s:5550%s' % (host, uri), verify = DOMAIN_PKI_REPO[host], timeout=timeout) 
			else :
				r = session.request(method, 'https://%s:5550%s' % (host, uri), verify = DOMAIN_PKI_REPO[host], json = param, timeout=timeout) 
